...
export function menuBarVariants(_props: KumoMenuBarVariantsProps = {}) {
  return cn(
    // Base styles
    "flex rounded-lg border border-kumo-fill bg-kumo-fill ps-px shadow-xs transition-colors",
  );
}
...
const MenuOption = ({
  ...
  return (
    <Tooltip content={tooltip} asChild>
      <button
        className={cn(
          "focus:inset-ring-focus relative -ms-px flex h-full w-11 cursor-pointer items-center justify-center rounded-md border-none bg-kumo-fill first:rounded-s-lg last:rounded-e-lg transition-colors focus:z-1 focus:outline-none focus-visible:z-1 focus-visible:inset-ring-[0.5]",
          {
...
export const MenuBar = ({
  ...
  return (
    <nav
      className={cn(
        "isolate flex rounded-lg border border-kumo-fill bg-kumo-fill ps-px shadow-xs transition-colors",
        className,
      )}
      ref={menuRef}
    >
