...
<Heading level={3}>Top Position</Heading>
<p>
  The <code>Toasty</code> wrapper positions toasts in the bottom-right by default.
  To place toasts at the top of the viewport, use the Base UI <code>Toast</code> primitives
  directly (re-exported from <code>@cloudflare/kumo</code>) with custom viewport and root styling.
  The key differences from bottom positioning are: anchor to <code>top: 0</code> instead of <code>bottom: 0</code>,
...
// Viewport: anchor to top instead of bottom
<Toast.Viewport className="fixed top-4 right-0 bottom-auto left-0 z-50 mx-auto flex w-full max-w-[340px]">

// Root: anchor top-0, origin-top, swipe up to dismiss
<Toast.Root
  toast={toast}
  swipeDirection="up"
  className={cn(
    "absolute top-0 left-0 right-0 mx-auto origin-top ...",
