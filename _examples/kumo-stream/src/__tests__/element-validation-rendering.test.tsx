...
function SpyComponent(props: any): React.JSX.Element {
...
beforeEach(() => {
  for (const t of spyTypes) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    COMPONENT_MAP[t] = SpyComponent as React.ComponentType<any>;
  }
});
...
afterEach(() => {
  for (const t of spyTypes) {
    if (originals[t] !== undefined) {
      COMPONENT_MAP[t] = originals[t]!;
    } else {
      delete COMPONENT_MAP[t];
    }
  }
  cleanup();
  vi.restoreAllMocks();
