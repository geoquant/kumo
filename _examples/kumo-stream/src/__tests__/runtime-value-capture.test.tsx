...
import { createRuntimeValueStore } from "../core/runtime-value-store";
...
const ORIGINAL_INPUT = COMPONENT_MAP.Input;
const ORIGINAL_TEXTAREA = COMPONENT_MAP.Textarea;

beforeEach(() => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  COMPONENT_MAP.Input = MockInput as React.ComponentType<any>;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  COMPONENT_MAP.Textarea = MockTextarea as React.ComponentType<any>;
});

afterEach(() => {
  if (ORIGINAL_INPUT !== undefined) {
    COMPONENT_MAP.Input = ORIGINAL_INPUT;
  } else {
    delete COMPONENT_MAP.Input;
  }

  if (ORIGINAL_TEXTAREA !== undefined) {
    COMPONENT_MAP.Textarea = ORIGINAL_TEXTAREA;
  } else {
    delete COMPONENT_MAP.Textarea;
  }
