...
const asComponent = <T>(component: T): ComponentType<any> =>
...
export const CardAdapter = createAdapter(asComponent(Card), {
  mapProps: (a2ui, ctx) => ({
    ...
    withBorder:
      extractValue(a2ui.withBorder) ?? extractValue(a2ui.bordered) ?? true,
    children: ctx.children,
  }),
...
export const TextAdapter = createAdapter(asComponent(Text), {
  mapProps: (a2ui, ctx) => {
    ...
    const usageHintStyles: Record<
      string,
      { size?: string; fw?: number; c?: string }
    > = {
      h1: { size: "2rem", fw: 700 },
    ...
    const hintStyle =
      (usageHint && usageHint in usageHintStyles
        ? usageHintStyles[usageHint]
        : null) ?? {};

    return {
      size: extractValue(a2ui.size) ?? hintStyle.size ?? "md",
      fw:
        extractValue(a2ui.weight) ??
        extractValue(a2ui.fontWeight) ??
        hintStyle.fw,
      ...
      td: extractValue(a2ui.underline)
        ? "underline"
        : extractValue(a2ui.strikethrough)
          ? "line-through"
          : undefined,
...
export const TitleAdapter = createAdapter(asComponent(Title), {
  mapProps: (a2ui, ctx) => {
    const rawContent =
      a2ui.text ?? a2ui.content ?? a2ui.label ?? a2ui.title ?? ctx.children;
    const content = extractValue(rawContent);
...
export const CodeAdapter = createAdapter(asComponent(Code), {
  mapProps: (a2ui, ctx) => {
    const content =
      extractValue(a2ui.code) ??
      extractValue(a2ui.text) ??
      extractValue(a2ui.content) ??
      ctx.children;
    return {
...
export const BlockquoteAdapter = createAdapter(asComponent(Blockquote), {
  mapProps: (a2ui, ctx) => {
    const content =
      extractValue(a2ui.quote) ??
      extractValue(a2ui.text) ??
      extractValue(a2ui.content) ??
      ctx.children;
    return {
      cite:
        extractValue(a2ui.cite) ??
        extractValue(a2ui.author) ??
        extractValue(a2ui.source),
...
export const HighlightAdapter = createAdapter(asComponent(Highlight), {
  mapProps: (a2ui, ctx) => {
    const content =
      ...
      "";
    const highlight =
      extractValue(a2ui.highlight) ?? extractValue(a2ui.query) ?? "";
    return {
      highlight: highlight,
...
export const MarkAdapter = createAdapter(asComponent(Mark), {
  mapProps: (a2ui, ctx) => {
    const content =
      extractValue(a2ui.text) ?? extractValue(a2ui.content) ?? ctx.children;
    return {
...
export const BadgeAdapter = createAdapter(asComponent(Badge), {
  mapProps: (a2ui) => ({
    children:
      ...
      extractValue(a2ui.content),
    color: mapVariant(
      extractValue(a2ui.variant) ?? extractValue(a2ui.color),
      {
        primary: "blue",
        secondary: "gray",
        success: "green",
    ...
    variant: extractValue(a2ui.filled)
      ? "filled"
      : extractValue(a2ui.outline)
        ? "outline"
        : "light",
    size: extractValue(a2ui.size) ?? "md",
...
export const IndicatorAdapter = createAdapter(Indicator, {
  mapProps: (a2ui, ctx) => ({
    ...
    processing:
      extractValue(a2ui.processing) ?? extractValue(a2ui.pulse) ?? false,
...
export const AvatarAdapter = createAdapter(asComponent(Avatar), {
  mapProps: (a2ui) => ({
    src:
      extractValue(a2ui.src) ??
      extractValue(a2ui.imageUrl) ??
      extractValue(a2ui.image),
    alt: extractValue(a2ui.alt) ?? extractValue(a2ui.name),
    radius: extractValue(a2ui.radius) ?? "xl",
    size: extractValue(a2ui.size) ?? "md",
    color: extractValue(a2ui.color) ?? "blue",
    children:
      extractValue(a2ui.initials) ??
      extractValue(a2ui.fallback) ??
      extractValue(a2ui.name)?.charAt(0),
  }),
...
/**
 * Extract plain text from a resolved A2UI child node.
 * Rendering Mantine's Text component inside Mantine's Button causes a crash
 * because of internal style context conflicts (the 'size' prop resolution).
 * Instead we extract the string content and pass it directly as children.
 *
 * Handles both direct Text children and nested structures like Row > Text
 * (used by submit-button snippets that wrap icon + text in a Row).
 */
function extractChildText(child: unknown): string | null {
  ...
  if (node.type === "Text" && props) {
    return (
      extractValue(props.text) ??
      extractValue(props.content) ??
      extractValue(props.label) ??
      null
    );
  }

  // Container node (Row, Column, etc.) — look for Text in children
  if (props?.children && Array.isArray(props.children)) {
    for (const grandchild of props.children) {
  ...
  if (props?.child) {
    return extractChildText(props.child);
  }
...
export const ButtonAdapter = createAdapter(asComponent(Button), {
  mapProps: (a2ui, ctx) => {
    // Extract text from a2ui.label/text, or from a child Text node directly.
    // Avoid passing pre-rendered Mantine <Text> as children of Mantine <Button>
    // because it crashes due to internal style context conflicts.
    const label =
      extractValue(a2ui.label) ??
      extractValue(a2ui.text) ??
      extractChildText(a2ui.child);
    ...
    return {
      children:
        label ?? (typeof ctx.children === "string" ? ctx.children : "Button"),
      onClick: createActionHandler(a2ui.action, ctx),
      variant: mapVariant(
        variant,
        {
          // Semantic names → Mantine variants
          primary: "filled",
          secondary: "outline",
          tertiary: "subtle",
          ghost: "subtle",
          ...
          destructive: "filled",
          success: "filled",
          // Mantine-native variant names (pass-through)
          filled: "filled",
          outline: "outline",
          subtle: "subtle",
      ...
      radius: extractValue(a2ui.radius) ?? "md",
      fullWidth:
        extractValue(a2ui.fullWidth) ?? extractValue(a2ui.block) ?? false,
      leftSection: IconComponent
        ? React.createElement(IconComponent, { size: 16 })
        : undefined,
    };
  },
  // Prevent auto-injection of pre-rendered children; we extract text from the child node directly
  childrenProp: null,
...
export const ActionIconAdapter = createAdapter(asComponent(ActionIcon), {
  mapProps: (a2ui, ctx) => {
    ...
    return {
      ...
      loading: extractValue(a2ui.loading) ?? false,
      "aria-label":
        extractValue(a2ui.label) ?? extractValue(a2ui.ariaLabel) ?? iconName,
...
export const TextFieldAdapter = createAdapter(TextInput, {
  mapProps: (a2ui, ctx) => {
    ...
    return {
      ...
      description:
        extractValue(a2ui.description) ?? extractValue(a2ui.helperText),
...
export const TextAreaAdapter = createAdapter(Textarea, {
  mapProps: (a2ui, _ctx) => {
    ...
    const isLiteralText =
      textValue !== undefined && typeof textValue !== "object";
    ...
    return {
      ...
      defaultValue: isLiteralText
        ? textValue
        : (extractValue(a2ui.defaultValue) ?? extractValue(a2ui.value)),
      styles: inputLabelStyles,
    };
  },
...
function DateTimeInputComponent({
  ...
  return React.createElement(asComponent(Box), {}, [
    label &&
      React.createElement(
        asComponent(Text),
        { key: "label", size: "sm", fw: 400, mb: 4 },
        [
          label,
          required &&
            React.createElement(
              asComponent(Text),
              { key: "req", span: true, c: "red", ml: 2 },
              "*",
            ),
        ],
      ),
...
export const DateTimeInputAdapter = createAdapter(
  DateTimeInputComponent as ComponentType,
  {
    mapProps: (a2ui, ctx) => ({
      label: extractValue(a2ui.label),
...
export const CheckboxAdapter = createAdapter(Checkbox, {
  mapProps: (a2ui, ctx) => ({
    ...
    description: extractValue(a2ui.description),
    defaultChecked:
      extractValue(a2ui.checked) ?? extractValue(a2ui.defaultChecked) ?? false,
    disabled: extractValue(a2ui.disabled) ?? false,
  ...
  mapProps: (a2ui, ctx) => {
    ...
    const data = rawOptions
      ...
      .map((opt: any) => {
        ...
        const rawValue =
          extractValue(opt.value) ??
          extractValue(opt.id) ??
          extractValue(opt.label) ??
          "";
        const value = String(rawValue);
        const rawLabel =
          extractValue(opt.label) ??
          extractValue(opt.text) ??
          extractValue(opt.value) ??
          value;
...
export const SegmentedControlAdapter = createAdapter(SegmentedControl, {
  mapProps: (a2ui, ctx) => {
    const data = (
      extractValue(a2ui.options) ??
      extractValue(a2ui.data) ??
      []
    ).map((opt: any) =>
      typeof opt === "string"
        ? { value: opt, label: opt }
...
export const ChipAdapter = createAdapter(asComponent(Chip), {
  mapProps: (a2ui, ctx) => ({
    children:
      extractValue(a2ui.label) ?? extractValue(a2ui.text) ?? ctx.children,
    defaultChecked:
      extractValue(a2ui.checked) ?? extractValue(a2ui.selected) ?? false,
...
export const SliderAdapter = createAdapter(Slider, {
  mapProps: (a2ui, ctx) => ({
    ...
    step: extractValue(a2ui.step) ?? 1,
    defaultValue:
      extractValue(a2ui.defaultValue) ?? extractValue(a2ui.value) ?? 50,
    disabled: extractValue(a2ui.disabled) ?? false,
    marks: extractValue(a2ui.marks),
    color: extractValue(a2ui.color),
    size: extractValue(a2ui.size) ?? "sm",
    showLabelOnHover: extractValue(a2ui.showLabel) ?? true,
    onChangeEnd: a2ui.onChange
      ? (value: number) => {
          ctx.onAction({
            actionName: a2ui.onChange.name,
            sourceComponentId: ctx.componentId,
...
export const RatingAdapter = createAdapter(Rating, {
  mapProps: (a2ui, ctx) => ({
    defaultValue:
      extractValue(a2ui.defaultValue) ?? extractValue(a2ui.value) ?? 0,
    count: extractValue(a2ui.count) ?? extractValue(a2ui.max) ?? 5,
    fractions:
      extractValue(a2ui.fractions) ?? (extractValue(a2ui.half) ? 2 : 1),
    size: extractValue(a2ui.size) ?? "md",
    color: extractValue(a2ui.color) ?? "yellow",
    readOnly: extractValue(a2ui.readOnly) ?? false,
...
export const AlertAdapter = createAdapter(Alert, {
  mapProps: (a2ui, ctx) => {
    ...
    const variant =
      extractValue(a2ui.variant) ?? extractValue(a2ui.type) ?? "info";
    ...
    return {
      ...
      withCloseButton:
        extractValue(a2ui.closable) ??
        extractValue(a2ui.withCloseButton) ??
        false,
      children:
        extractValue(a2ui.message) ??
        extractValue(a2ui.description) ??
        extractValue(a2ui.content) ??
        ctx.children,
    };
  },
...
export const ProgressAdapter = createAdapter(Progress, {
  mapProps: (a2ui) => ({
    ...
    animated:
      extractValue(a2ui.animated) ?? extractValue(a2ui.indeterminate) ?? false,
  }),
...
export const RingProgressAdapter = createAdapter(RingProgress, {
  mapProps: (a2ui) => {
    ...
    return {
      ...
      label:
        extractValue(a2ui.showLabel) !== false
          ? React.createElement(
              asComponent(Text),
              { size: "lg", ta: "center", fw: 700 },
              `${value}%`,
            )
          : undefined,
    };
...
export const TabsAdapter = createAdapter(Tabs, {
  mapProps: (a2ui, ctx) => ({
    defaultValue:
      extractValue(a2ui.defaultValue) ?? extractValue(a2ui.defaultTab),
    orientation: extractValue(a2ui.orientation) ?? "horizontal",
    variant: extractValue(a2ui.variant) ?? "default",
    color: extractValue(a2ui.color),
    children: ctx.children,
...
export const BreadcrumbAdapter = createAdapter(Breadcrumbs, {
  mapProps: (a2ui, ctx) => {
    const items = (extractValue(a2ui.items) ?? []).map(
      (item: any, idx: number) =>
        React.createElement(
          asComponent(Anchor),
          { href: item.href ?? "#", key: idx, size: "sm" },
...
export const MantineListAdapter = createAdapter(MantineList, {
  mapProps: (a2ui, ctx) => ({
    type: (extractValue(a2ui.ordered) ? "ordered" : "unordered") as
      | "ordered"
      | "unordered",
    size: extractValue(a2ui.size) ?? "md",
...
export const TableAdapter = createAdapter(Table, {
  mapProps: (a2ui, ctx) => ({
    ...
    withTableBorder:
      extractValue(a2ui.bordered) ?? extractValue(a2ui.withBorder) ?? false,
...
export const TableCellAdapter = createAdapter(Table.Td, {
  mapProps: (a2ui, ctx) => ({
    children:
      extractValue(a2ui.content) ?? extractValue(a2ui.text) ?? ctx.children,
  }),
  ...
  mapProps: (a2ui, ctx) => ({
    ...
    children: React.createElement(React.Fragment, {}, [
      React.createElement(
        Accordion.Control,
        { key: "control" },
        extractValue(a2ui.title) ?? extractValue(a2ui.label),
      ),
...
export const AnchorAdapter = createAdapter(asComponent(Anchor), {
  mapProps: (a2ui, ctx) => ({
    ...
    target:
      extractValue(a2ui.target) ??
      (extractValue(a2ui.external) ? "_blank" : undefined),
