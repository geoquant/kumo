...
import { kumoComponents } from "@a2ui-bridge/react-kumo";
import {
  StreamingProgress,
...
const SCENARIOS = [
  {
    ...
    label: "Doctor Visit",
    prompt:
      "I need to schedule a follow-up appointment with my doctor about my prescription refill",
  },
  {
    icon: ListChecks,
    label: "Get Organized",
    prompt:
      "I've got a million things to do today and need to get my thoughts organized into a manageable task list",
  },
  {
    icon: ChefHat,
    label: "Find Recipe",
    prompt:
      "I want to bake chocolate chip cookies this weekend and need to find a good recipe",
  },
  {
    icon: Plane,
    label: "Plan Trip",
    prompt:
      "I'm planning a weekend getaway and need to find and book a hotel room",
  },
  {
    icon: Banknote,
    label: "Send Money",
    prompt:
      "I need to send $50 to my roommate for my share of the utilities bill",
  },
  {
    icon: Target,
    label: "Track Goals",
    prompt:
      "Help me track my fitness goals for this week - I want to run 3 times and drink more water",
  },
];
...
export function Demo() {
  ...
  const [isDark, setIsDark] = useState(() => {
    if (typeof window !== "undefined") {
      ...
      // Ensure data-mode is set on initial load for Kumo theme compatibility
  ...
  // Toggle dark mode — set both .dark class (ShadCN) and data-mode attribute (Kumo)
  const toggleDarkMode = useCallback(() => {
    const newIsDark = !isDark;
    setIsDark(newIsDark);
    document.documentElement.classList.toggle("dark", newIsDark);
    document.documentElement.dataset.mode = newIsDark ? "dark" : "light";
  }, [isDark]);
  ...
  const handleDownload = useCallback(() => {
    if (!parsedMessages.length) return;
    const blob = new Blob([JSON.stringify(parsedMessages, null, 2)], {
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <Badge
              variant="secondary"
              className="bg-[hsl(var(--brand-light))] text-[hsl(var(--brand))] border-0"
            >
              Predictive UI
            </Badge>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Tooltip>
              ...
              <TooltipContent>
                Switch between design systems to see the same UI rendered
                differently
              </TooltipContent>
            </Tooltip>
            ...
            <div className="p-4 flex flex-col flex-1 min-h-0">
              ...
              <div className="mt-4">
                ...
                {!Object.values(availableProviders).some(Boolean) && (
                  ...
                  >
                    Set API key in .env (VITE_ANTHROPIC_API_KEY,
              ...
              {!showPreview && !isGenerating && parsedMessages.length === 0 && (
                <div className="flex flex-col items-center gap-6">
                  ...
                  <div className="text-center">
                    ...
                    >
                      Describe what you need, and AI will create the perfect
                      interface to help you.
                    </p>
                  </div>
                  <Badge
                    variant="secondary"
                    className="bg-[hsl(var(--brand-light))] text-[hsl(var(--brand))] border-0 text-sm px-3 py-1"
                  >
                    Powered by {PROVIDERS[provider].name.split(" ")[0]} + A2UI
                    Protocol
                  </Badge>
                </div>
              )}
            ...
            <SheetHeader>
              <SheetTitle className="flex items-center gap-2">
                ...
                {parsedMessages.length > 0 && (
                  ...
                  >
                    {parsedMessages.length} message
                    {parsedMessages.length !== 1 ? "s" : ""}
                  </Badge>
                )}
              </SheetTitle>
            </SheetHeader>
