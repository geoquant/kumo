...
export const TextAdapter = createAdapter(asComponent(Text), {
  mapProps: (a2ui, ctx) => {
    ...
    // Guard: if content is empty/null, render nothing meaningful to avoid crashes
    if (content === null || content === undefined || content === "") {
      return { children: content ?? "", span: true, size: "md" };
    }
    ...
    // Defensive: Mantine's Text component can crash if `size` receives
    // an object instead of a string/number, so ensure it's a primitive.
    const rawSize = extractValue(a2ui.size) ?? hintStyle.size ?? "md";
    const size =
 ...
 * This prevents rendering Mantine Text inside Mantine Button,
 * which causes style context conflicts with the 'size' prop.
 */
...
export const ButtonAdapter = createAdapter(asComponent(Button), {
  mapProps: (a2ui, ctx) => {
    // Extract text directly from child Text node to avoid rendering Mantine Text
    // inside Mantine Button (which causes style context conflicts with 'size').
    const label =
      extractValue(a2ui.label) ??
      extractValue(a2ui.text) ??
    ...
    return {
      // Use extracted label string; fall back to string "Button" to avoid rendering
      // Mantine Text inside Mantine Button (which causes 'size' context crash).
      children:
  ...
  childrenProp: null, // Don't auto-inject pre-rendered children; we extract text from child node directly
